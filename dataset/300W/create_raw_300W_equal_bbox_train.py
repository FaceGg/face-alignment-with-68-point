...
if __name__ == "__main__":
    ...
    parent_dir = '/home/troila/luobo/jms_test/6/face_alignment-master/dataset/300W'
    out_dir = '/home/troila/luobo/jms_test/6/face_alignment-master/dataset/dataset'

    # global parameters
    num_kpts = 68
    text_file = open("300W_trainset.txt", "w")

    # Read lfpw dataset
    train_dir_lfpw = "%s/lfpw/trainset" %(parent_dir)
	...
	# crop face box
        ...
        # output bbox
        # here the x and y order is reversed, be careful!!!
    ...
    # Read helen dataset
    train_dir_helen = "%s/helen/trainset" %(parent_dir)
    #test_dir = "%s/helen/testset" %(parent_dir)

    # read pts file
    path_helen = "%s" %(train_dir_helen)
    files_helen = os.listdir(path_helen)
    files_helen = [i for i in files_helen if i.endswith('.pts')]

    for index, file_pts in enumerate(files_helen):
        file_path = "%s/%s" %(path_helen, file_pts)
        kpts =  get_kpts(file_path)
        ...
        file_jpg = file_pts.split('.')[0] + '.jpg'
        jpg_path =  "%s/%s" %(path_helen, file_jpg)
        if not os.path.isfile(jpg_path):
            file_jpg = file_pts.split('.')[0] + '.png'
            jpg_path =  "%s/%s" %(path_helen, file_jpg)
    ...
    # Read AFW dataset
    train_dir_afw = "%s/afw" %(parent_dir)
    #test_dir = "%s/helen/testset" %(parent_dir)

    # read pts file
    path_afw = "%s" %(train_dir_afw)
    files_afw = os.listdir(path_afw)
    files_afw = [i for i in files_afw if i.endswith('.pts')]

    for index, file_pts in enumerate(files_afw):
        file_path = "%s/%s" %(path_afw, file_pts)
        kpts =  get_kpts(file_path)
        ...
        file_jpg = file_pts.split('.')[0] + '.jpg'
        jpg_path =  "%s/%s" %(path_afw, file_jpg)
        if not os.path.isfile(jpg_path):
            file_jpg = file_pts.split('.')[0] + '.png'
            jpg_path =  "%s/%s" %(path_afw, file_jpg)
